<template>
    <section class="h-full bg-transparent">
        <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0 bg-transparent">
            <div @click="goTo('login')" class="flex items-center mb-6 text-2xl font-semibold text-gray-900 dark:text-white">
            ...
            <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:bg-opacity-40 dark:border-gray-700">
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form @submit.prevent="login" class="space-y-4 md:space-y-6" action="#" enctype="multipart/form-data">
                        <div>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-start">
                                <div class="flex items-center h-5">
                                    <input v-model="user.rememberMe" id="remember" aria-describedby="remember" type="checkbox" class="w-4 h-4 border border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-blue-300 dark:bg-opacity-20 dark:bg-gray-700 dark:border-gray-600 dark:focus:ring-blue-600 dark:ring-offset-gray-800">
                                </div>
                                <div class="ml-3 text-sm">
                                    <label for="remember" class="text-gray-500 dark:text-gray-300">Remember me</label>
                                </div>
                            </div>
                            <span @click="goTo('passwordReset')" class="text-sm font-medium text-blue-600 hover:underline dark:text-blue-500">Forgot password?</span>
                        </div>
                        ...
                        <p class="text-sm font-light text-gray-500 dark:text-gray-400">
                            Don’t have an account yet? <span @click="goTo('signup')" class="font-medium cursor-pointer text-blue-600 hover:underline dark:text-blue-500">Sign up</span>
                        </p>
                    </form>
                </div>
            </div>
    ...
    export default {
        setup(){
            ...
            const login = async () => {
                ...
                store.login(data).then(() => router.push({name: 'dashboard'}))
            }

            return { goTo, validator, login, user }
        }
    }
</script>
